...
import { groupDashboard, createGroup, check_getGroup } from "./groups";
...
app.post("/rest/newGroup", async (req, res) => {
  if (!isAuthenticated(req, res)) return;

  if (!(await check_getGroup(db, req, res))) {
    res.redirect("/groups?pagestatus=Group already exists.");
    return;
  }
